...
export default function WishTable() {
    ...
    const wishTableList = wishData.map((item, idx) => (
        <tr key={idx}>
            ...
            <td className="product-price-cart"><span className="amount old">{item.price}{(item.price * ((100-item.discount)/100)).toFixed(2)}</span><span className="amount">{item.price}</span></td>
