...
export default function SignUp() {
    ...
    const [guideTxts, setGuideTxts] = useState({
        ...
        pwdGuide : '숫자와 문자를 조합해 8글자 이상 입력해 주세요.',
        confirmPwdGuide : 'PASSWORD를 한번 더 입력해 주세요.',
        nameGuide : '',
        phoneGuide : '-(하이폰)을 입력하지 말아 주세요.    ex) 01012345678'
    });
  ...
  const isPwd = pass => {
    const pwdRegex = /^.*(?=.{8,20})(?=.*[0-9])(?=.*[a-zA-Z])(?=.*[@$!%*#?&]).*$/;
  ...
  const confirmPassword = (pass, confirmPass) => {
    ...
    let process = require('../../../Process.json');
