...
export default function CompareTable() {
    ...
    const compareList01 = compareData.map(item => (
    ...
    const compareList02 = compareData.map(item => (
    ...
    const compareList03 = compareData.map(item => (
        <td className="product-desc">
            <p>{item.shortDescription}</p>
        </td>
    )).slice(0,3);

    const compareList04 = compareData.map(item => (
